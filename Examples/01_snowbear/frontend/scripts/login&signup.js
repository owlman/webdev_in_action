...
loginForm.username.addEventListener('focus', function() {
    ...
    formCheck.removeHint(loginForm, 'checkname');
});
// 在用户名文本框失去焦点时检查输入
loginForm.username.addEventListener('blur', function() {
    formCheck.checkUsername(loginForm);
});
// 在密码文本框获得焦点时移除输入提示信息
loginForm.password.addEventListener('focus', function() {
    this.classList.remove('is-invalid');
    this.classList.remove('is-valid');
    formCheck.removeHint(loginForm, 'checkpwd');
});
// 在密码文本框失去焦点时检查输入
loginForm.password.addEventListener('blur', function() {
    formCheck.checkPassword(loginForm, 'checkpwd');
});
// 提交表单时的处理逻辑
...
loginForm.addEventListener('reset', function() {
    // 清除表单输入提示
    formCheck.removeHint(this, 'checkname');
    formCheck.removeHint(this,'checkpwd');
    // 清除表单验证样式;
    formCheck.removeValidation(loginForm.username);
    formCheck.removeValidation(loginForm.password);
    // 重置表单
    loginForm.reset();
})
...
signupForm.username.addEventListener('foucs', function() {
    this.classList.remove('is-invalid');
    this.classList.remove('is-valid');
    formCheck.removeHint(signupForm, 'checkname')
});
// 在用户名文本框失去焦点时检查输入
signupForm.username.addEventListener('blur', function() {
    formCheck.checkUsername(signupForm);
});
// 在密码文本框获得焦点时移除输入提示信息
...
signupForm.password.addEventListener('blur', function() {
    formCheck.checkPassword(signupForm, 'checkpwd');
});
// 在用于确认密码的文本框获得焦点时移除输入提示信息
signupForm.confirmpwd.addEventListener('focus', function() {
    this.classList.remove('is-invalid');
    this.classList.remove('is-valid');
    formCheck.removeHint(signupForm, 'confirmpwd');
})
...
signupForm.confirmpwd.addEventListener('blur', function() {
    if (!this.value) {
        ...
        formCheck.addHint(signupForm,
                'confirmpwd', '请确认您的密码！');
    } else if(this.value !== signupForm.password.value) {
        // 确认密码与密码不一致时的处理逻辑
        this.classList.add('is-invalid');
        formCheck.addHint(signupForm, 
            'confirmpwd',  '您输入的确认密码与之前不一致！');
    } else {
        // 确认密码输入合法时的处理逻辑
        this.classList.remove('is-invalid');
        formCheck.removeHint(signupForm, 
            'confirmpwd');
...
signupForm.addEventListener('submit', async function(event) {
    ...
    if (!signupForm.username.value 
        ...
        // 用户名或密码为空时的处理逻辑
        formCheck.checkUsername(signupForm);
        formCheck.checkPassword(signupForm);
        return;
...
signupForm.addEventListener('reset', function() {
    ...
    formCheck.removeHint(this, 'checkname');
    formCheck.removeHint(this, 'checkpwd');
    formCheck.removeHint(this, 'confirmpwd');
    // 清除表单验证样式;
    formCheck.removeValidation(signupForm.username);
    formCheck.removeValidation(signupForm.password);
    formCheck.removeValidation(signupForm.confirmpwd);
    // 重置表单
    signupForm.reset();
})
