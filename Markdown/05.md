...
本章项目将会致力于演示如何为一家企业的官方网站实现其线上购物功能的Web UI，以便其用户可以直接通过网页浏览器来享用其线上点餐服务，以便更好地借助互联网来开展该连锁店的业务。通过本章项目的实践，读者将会初步学会如何实现订单管理、线上点餐这两大功能的Web UI，并掌握实现这些功能所需要掌握的技术和相关工具。总而言之，在阅读完本章之后，我们希望读者能够：
  ...
  - *用于获取饮料菜单的API*：
    ...
    - *用于删除指定订单的API*：
      ...
      - 响应数据，以JSON格式返回：
        ...
        - 失败响应2: `{ status: 400, message: "request_url_error“}`
- **项目要求**：基于【任务书】提供的HTTP API构建出用于执行线上点餐与订单管理操作的Web UI，该UI的实现应符合以下要求。
  - 该Web UI应允许用户在完成支付之前在前端暂存想购买的饮料；
  - 该Web UI应允许用户对自己所要购买的饮料执行扫码支付操作；
  - 该Web UI应允许用户在完成支付之后查看、删除购买过的饮料；
  **时间要求**：在20个工作日内完成；
...
有一家名为“白熊前端”的程序员培训机构，他们决定为自己现有的Web服务添加一个用于参加付费课程的功能模块，以便用户可以通过线上缴费的方式参与其提供的职业培训课程，在开发该功能模块的项目中，你的任务是根据项目组中负责后端部分的成员所实现的HTTP API来构建用于执行线上缴费与付费课程管理的Web UI。

- **项目名**：白熊培训网站用户信息管理的Web UI
- **委托方**：白熊前端的创始人：林宇一
- **项目资料**：用户付费课程功能的后端API，其具体信息如下。
  - *用于获取付费课程列表的API*：
    - 请求URL：`http://whitebear.com/courses/menu`
    - 请求方法：`GET`
    - 响应数据，以JSON格式返回：
      - 成功响应：`{ status: 200, orderLIst: <以JSON格式返回的菜单列表>}`
      - 失败响应1: `{ status: 404, message: "course_list_failed"}`
      - 失败响应2: `{ status: 400, message: "request_url_error"}`
  - *用于查看指定已购课程的API*：
    - 请求URL：`http://whitebear.com/courses/<用户的ID>&<课程的ID>`
    - 请求方法：`GET`
    - 响应数据，以JSON格式返回：
      - 成功响应：`{ status: 200, message: "course_get_success"}`
      - 失败响应1: `{ status: 500, message: "course_get_failed"}`
      - 失败响应2: `{ status: 400, message: "request_url_error“}`
  - *用于查看所有已购课程的API*：
    - 请求URL：`http://whitebear.com/courses/<用户的ID>`
    - 请求方法：`GET`
    - 响应数据，以JSON格式返回：
      - 成功响应：`{ status: 200, orderLIst: <以JSON格式返回的订单列表>}`
      - 失败响应1: `{ status: 404, message: "course_list_failed"}`
  ...
  - *用于参加新付费课程的API*：
    ...
    - 请求参数，需以JSON格式提交, 具体数据为新订单的数据信息。
    - 响应数据，以JSON格式返回：
      - 成功响应：`{ status: 200, message: "course_add_success"}`
      - 失败响应1: `{ status: 403, message: "course_add_failed"}`
      - 失败响应2: `{ status: 400, message: "request_url_error"}`
  - *用于退出指定付费课程的API*：
    - 请求URL：`http://whitebear.com/courses/<用户的ID>&<课程的ID>`
    - 请求方法：`DELETE`
    - 响应数据，以JSON格式返回：
      ...
      - 失败响应1: `{ status: 500, message: "order_delete_failed"}`
      - 失败响应2: `{ status: 400, message: "request_url_error“}`
- **项目要求**：基于【任务书】提供的HTTP API构建出用于参加付费课程的Web UI，该UI的实现应符合以下要求。
  - 该Web UI应允许用户在完成支付之前在前端暂存想参加的付费课程；
  - 该Web UI应允许用户对自己所要购买的付费课程执行扫码支付操作；
  - 该Web UI应允许用户在完成支付之后查看、退出自己所参加的课程；
  **时间要求**：在20个工作日内完成；
...
| ----- | --------- | ------------ | --- | ------ | ------ | ---- | --------- |
| 01 | Web UI的可访问性 | Web UI能否通过甲方提供的URL进行访问？| 20 |  |  |  |  |  
| 02 | 课程管理功能 |Web UI能否支持用户管理自己所参加的付费课程？| 40 |  |  |  |   |
| 03 | 线上缴费功能 | Web UI能否支持用户执行参加指定课程的缴费操作？| 40   |  |  |  |  |
