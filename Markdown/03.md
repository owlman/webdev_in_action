...
5. 在保存了上述代码之后，读者可以试着重新启动一下该Web应用的前端服务，然后再用浏览器打开当前步骤所创建的`login&signup.htm`文件，就可以看到它所呈现的界面效果了，其效果如图3-2所示。
...
3. 在保存了上述代码之后，读者可以试着重新启动一下该Web应用的前端服务，然后再用浏览器打开刚刚修改过的`login&signup.htm`文件，就可以看到它所呈现的界面效果了，其效果如图3-3所示。
