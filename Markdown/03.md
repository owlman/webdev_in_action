...
- **`querySelector()`方法**：用于返回目标对象中第一个符合条件的元素对象，它接收一个字符串类型的调用参数，读者可以通过该参数用`#id`、`.className`、`tagName`这样的CSS选择器模式来指定要匹配的元素，例如像下面这样。
...
- **`querySelectorAll()`方法**：用于返回目标对象中所有符合条件的元素对象，它也同样接收一个字符串类型的调用参数，该参数的用法与`querySelector()`方法相同，也就是说，我们可以这样使用该接口：
...
- **`upload`属性**：只读属性，作用是返回一个用来表示上传进度的`XMLHttpRequestUpload`对象，我们可以通过响应该对象事件的方式来追踪数据上传的进度。下面我们就具体来介绍一下可在该对象上注册的一些事件处理函数：
  - **`onloadstart`事件处理函数**：用于响应数据上传操作的启动事件，将在数据上传开始时被调用。
  - **`onprogress`事件处理函数**：用于响应数据上传的这个操作过程，换而言之，它会在数据上传的过程中被持续调用，所以建议不要在其中执行太复杂的操作。
  - **`onabort`事件处理函数**：用于响应数据上传操作的中止事件，将在数据上传过程被中止时被调用。
  - **`onerror`事件处理函数**：用于响应数据上传操作的出错事件，将在数据上传失败时被调用。
  - **`onload`事件处理函数**：用于响应数据上传操作的完成事件，将在数据上传成功时被调用。
  - **`ontimeout`事件处理函数**：用于响应数据上传操作的超时事件，将在数据上传未在用户规定的时间内完成时被调用。
  - **`onloadend`事件处理函数**：用于响应数据上传操作的终止事件，将在数据上传的操作完成时被调用（不论上传成功与否）。
...
- **`getAllResponseHeaders()`方法**：以字符串的形式返回所有用CRLF分隔的响应头，如果请求没有收到响应，则返回`null`。
- **`getResponseHeader()`方法**：用于返回包含指定响应头的字符串，如果请求没有收到响应或在收到的响应中不存在该报头，则返回 null。
- **`open()`方法**：用于初始化`XMLHttpRequest`对象要发送的请求。
- **`overrideMimeType()`方法**：用于重写由服务器返回的MIME类型。
- **`send()`方法**：该方法用于发送请求。如果请求是异步的（默认），那么该方法将在请求发送后立即返回。
- **`setRequestHeader()`方法**：用于设置HTTP请求头的值，它必须在`open()`之后、`send()`之前被调用。
- **`onreadystatechange`事件处理函数**：将会在其所在`XMLHttpRequest`对象的`readyState`属性值发生变化时被调用。
...
在完成了上述工作准备之后，读者现在就可以根据之前【任务书】中的要求来着手为凌雪冰熊网站实现用户注册/登录功能的前端部分了，该项目的实施过程可以分为以下步骤来进行。
...
5. 在保存了上述代码之后，读者可以试着重新启动一下该Web应用的前端服务，然后再用网页浏览器打开当前步骤所创建的`login&signup.htm`文件，就可以看到它所呈现的界面效果了。例如，图3-2中所示的是该Web UI在用户输入不符合规则时的显示效果。
...
- 首先，读者需要回到本章项目Web UI所在的目录中（即`01_snowbear/frontend/static`目录），将`blueimp-md5`库下载到该目录中，具体命令如下。

    ```bash
    npm install blueimp-md5 --save
    ```

- 接下来，读者需要回到之前所创建的`login&signup.htm`文件中，并其`<head>`与`</head>`这对标记之间，以及引入`login&signup.js`脚本的`<script>`标记之前添加如下代码，以便将`blueimp-md5`库引入到当前脚本中。
...
- 继续在`login&signup.js`文件中，找到`loginForm`、`signupForm`这两个表单的`submit`事件处理函数，并将其中收集表单数据的代码修改如下。
...
- 最后，读者需要将当前步骤所获得的进展提交给git版本控制系统了，具体命令如下。
...
- 首先，使用VS Code编辑器重新打开`01_snowbear`项目，并在项目根目录下创建一个名为`test`的子目录，以便用于安装这个测试框架。

- 接下来，读者需要使用命令行终端进入到刚刚创建的`test`目录中，将`cypress`库下载到该目录中，具体命令如下。

    ```bash
    npm install cypress --save
    ```

- 如果一切顺利，当我们继续在`test`目录中执行`npx cypress open`命令时，就会看到如图3-4所示的图形化向导界面了。

    ![Cypress界面](./img/3-4.png)

    **图3-4** Cypress图形化向导界面

- 下面来编写一下针对用户登录功能的测试用例。为此，读者需要先将上述界面关闭，然后在`test`目录中找到已经自动生成的`cypress/e2e`目录，并在创建一个名为`3-snowbear`的目录，以便用于存放本项目的端对端测试代码

- 继续进入到刚刚创建的`3-snowbear`目录中，并创建一个名为`login.cy.js`的文件，并在其中输入如下测试代码。
...
- 在保存上述代码所在的文件之后，读者需要回到之前所创建的`03_snowbear/test`目录中，通过执行`npx cypress open`命令重新打开Cypress图形化界面，然后在界面中单击“E2E Testing”按钮，届时读者就会看到一个如图3-5所示的浏览器选择界面。

    ![浏览器选择界面](./img/3-5.png)

    **图3-5** 浏览器选择界面

- 继续在上述界面中单击“Start E2E Testing  in Chrome”按钮，读者就会看到一个如图3-6所示的测试脚本界面了。

    ![测试脚本界面](./img/3-6.png)

    **图3-6** 测试脚本界面

- 接下来，读者就只需在上述界面中找到自己之前创建的`login.cy.js`文件并单击它，就可以启动对本项目用户登录功能的测试了，其测试过程如图3-7所示。
...
必须要强调的是，`Cypress`与上一章所介绍的`superTest`库相比，是一个功能更为全面的测试框架，它提供的API更多，可以执行的测试种类也更为丰富，因而学习曲线也相对更为陡峭一些。本章在这里所演示的只是这个测试框架的基本使用步骤，如果读者想更全面地掌握它的使用方法，还需要去仔细阅读一下`Cypress`框架的官方文档（如图3-8所示）。

![Cypress官方文档](./img/3-8.png)

**图3-8** Cypress官方文档

## 【作业】

有一家名为“白熊前端”的程序员培训机构，他们决定为自己现有的Web服务添加一个用户功能模块，以便用户可以通过互联网申请成为他们的学员，并能在登录之后享用他们的线上培训服务，在开发该功能模块的项目中，你的任务是根据项目组中负责后端部分的成员所实现的HTTP API来构建用于执行注册/登录操作的Web UI。
