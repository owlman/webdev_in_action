...
在这一步骤中，软件工程师的主要任务是基于项目【任务书】中提供的、用于执行用户注册/登录功能的后端HTTP API来创建相应的Web UI。为此，读者需要执行以下操作。

1. 使用VS Code编辑器打开之前创建的`01_snowbear`项目，然后在项目根目录下的`frontend/static`目录中找到名为`template.htm`的模板文件，并基于该模版创建一个名为`login&signup.htm`的HTML文件。

2. 打开刚刚创建的`login&signup.htm`文件，然后其中找到`<!-- 在此处填充网页的主体内容 -->`这行注释所在的位置，并在该位置上插入如下代码。

   ```html
   ```

3. 下面来编写一下用于处理上述HTML文档中表单元素的JavaScript脚本。为此，读者需要继续在上述`frontend/static`目录下创建一个名为`scripts`的子目录，并在其中创建一个名为`login&signup.js`的模块文件，并在其中输入如下代码。

   ```JavaScript
   ```

4. 在保存了上述代码之后，读者可以试着重新启动一下该Web应用的前端服务，然后再用浏览器打开当前步骤所创建的`login&signup.htm`文件，就可以看到它所呈现的界面效果了，其效果如图3-2所示。

    ![用户登录/注册界面效果](./img/3-2.png)

    **图3-2**：用户登录/注册界面效果

5. 接下来，读者需要将当前步骤所获得的进展提交给git版本控制系统了，具体命令如下。

    ```bash
    git add .
    git commit -m "项目3：创建用于执行用户登录/注册功能的Web UI"
    ```
...
1. 使用VS Code编辑器打开之前创建的`01_snowbear`项目，然后在项目根目录下的`frontend/static`目录中找到名为`template.htm`的模板文件，并基于该模版创建一个名为`messageEdit.htm`的HTML文件。

2. 打开刚刚创建的`messageEdit.htm`文件，然后其中找到`<!-- 在此处填充网页的主体内容 -->`这行注释所在的位置，并在该位置上插入如下代码。

   ```html
   ```

2. 下面来编写一下用于处理上述HTML文档中表单元素的JavaScript脚本。为此，读者需要继续在上述`frontend/static`目录下创建一个名为`scripts`的子目录，并在其中创建一个名为`messageEdit.js`的模块文件，并在其中输入如下代码。

   ```JavaScript
   ```

3. 在保存了上述代码之后，读者可以试着重新启动一下该Web应用的前端服务，然后再用浏览器打开当前步骤所创建的`messageEdit.htm`文件，就可以看到它所呈现的界面效果了，其效果如图3-2所示。

    ![用户个人信息编辑界面](./img/3-2.png)
...
4. 接下来，读者需要将当前步骤所获得的进展提交给git版本控制系统了，具体命令如下。
    ...
    ```bash
    git add .
    git commit -m "项目3：创建用于查看/编辑用户个人信息的Web UI"
    ```
