...
1. 使用VS Code编辑器打开之前创建的`01_snowbear`项目，然后在项目根目录下的`frontend/static`目录中为创建一个名为`login&signup.htm`的HTML文件，并在其中输入如下代码。

   ```html
   ```

2. 下面来编写一下用于处理上述HTML文档中表单元素的JavaScript脚本。为此，读者需要继续在上述`frontend/static`目录下创建一个名为`scripts`的子目录，并在其中创建一个名为`login&signup.js`的模块文件，并在其中输入如下代码。

   ```JavaScript
   ```

3. 在保存了上述代码之后，读者可以试着重新启动一下该Web应用的前端服务，然后再用浏览器打开当前步骤所创建的`login&signup.htm`文件，就可以看到它所呈现的界面效果了，其效果如图3-2所示。
...
1. 使用VS Code编辑器打开之前创建的`01_snowbear`项目，然后在项目根目录下的`frontend/static`目录中为创建一个名为`messageEdit.htm`的HTML文件，并在其中输入如下代码。
