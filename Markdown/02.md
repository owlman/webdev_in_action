...
在这一步骤中，软件工程师的主要任务是创建一个专用于在MongoDB数据库中操作`users`数据集的Node.js模块，该模块所提供的API将会在接下来为凌雪冰熊网站实现各项用户服务的过程中被反复调用，目的是通过对这部分代码的封装来减少项目整体的编码量，从而提高自己的工作效率。为此，读者需要执行以下操作。
...
3. 如果上述操作一切顺利，读者就可以将当前步骤所获得的进展提交给git版本控制系统了，具体命令如下。
    ...
    ```bash
    git add .
    git commit -m "项目2：创建专用于执行数据操作的API"
    ```
