...
请注意，上述表格中列出的“请求路径”并非是一个完整的URL。按照REST设计规范，完整的URL还应该包含调用AP 所使用的通信协议（通常是HTTP或HTTPS），API 所在服务器的域名与端口号等相关信息。除此之外，如果我们还想兼顾API未来被重构之后可能引发的向后兼容问题，有时候也会选择在URL中加入版本信息。例如，如果我们将API部署在`snowbear.com`这个域名下，服务器端口为`3000`，那么前端想获取`<用户的ID>`值为`14`的个人信息，它使用GET方法发送HTTP请求的URL就应该是像下面这样。

```bash
http://snowbear.com/v1/users/14
```

需要特别说明的是，开发者们在设计RESTful API时常常会下意识地犯一个设计理念上的错误，那就是将前端发送的URL设计成一个调用服务器函数的“动作”，例如在要获取指定用户发表的所有帖子时，他们极有可能将URL中的请求路径写成类似于`/users/query?uid=14`这种形式，毕竟他们在PHP、JSP的时代一直都是这么做的。但在REST设计规范中，表达调用的动作通常是由HTTP请求方法来传递的，URL只用来指定前端需要后端服务提供的“资源”，所以它应该是一系列的名词，而非动词。

最后，当以上API向前端返回响应数据时，除了必须采用JSON、XML等通用数据格式之外，还应该尽可能地使用不同的HTTP状态码来清晰地表示后端服务器不同的响应状态。下面是一些最常用的HTTP状态码以及它们分别所代表的含义。

- **200 OK**：用于表示请求已成功，请求所希望获取的响应头或数据体将随此响应数据返回。
- **201 Created**：用于表示请求已被实现，后端已经依据请求创建了相关数据，并将这些数据的URL以`Location`头信息的形式返回给前端。
- **204 No Content**：用于表示后端成功处理了请求，但响应动作没有返回任何内容。
- **205 Reset Content**：用于也表示后端成功处理了请求，但没有返回任何内容。与204状态码不同的是，发送该状态码的响应动作会要求发送请求的前端重置文档视图。
- **400 Bad Request**：用于表示由于某种明显的前端错误，导致了后端无法处理或识别该请求。
- **403 Forbidden**：用于表示后端已经理解请求，但拒绝处理它。
- **404 Not Found**：用于表示当前请求所希望得到的数据在后端不存在，或对用户不可见。
- **408 Request Timeout**：用于表示前端发出的请求已超时。根据HTTP协议的规范，如果前端没有在后端预设的等待时间内完成一个请求的发送，就需要再次提交这一请求。
- **410 Gone**：用于表示前端所请求的数据已被后端有意删除或清理，不可再被使用。
- **415 Unsupported Media Type**：用于表示前端在请求时所用的互联网媒体类型并不属于后端所支持的数据格式，因此该请求被拒绝处理。
- **500 Internal Server Error**：用于代表的是通用错误消息，即后端遇到了一个未曾预料的状况，该状况导致了它无法完成对请求的处理。在这种情况下，后端也无法给出具体错误信息。
- **501 Not Implemented**：用于表示后端不支持前端请求的某个功能。
- **503 Service Unavailable**：用于表示后端正在维护或出现了临时过载的问题，无法处理来自前端的请求。这个状况是暂时的，通常过一段时间就会恢复。
...
- **`createCollection()`方法**：用于创建一个新的数据集，它接收一个字符串类型的实参，用于指定新建数据集的名称。

- **`dropCollection()`方法**：用于删除某一个指定的数据集，它接收一个字符串类型的实参，用于指定要删除数据集的名称。

- **`collection()`方法**：用于指定当前要操作的数据集，它接收一个字符串类型的实参，用于指定该数据集。然后，我们可以在其返回的数据集连接上执行以下常用操作：
  - **`dbName()`方法**：用于返回当前数据集所属的数据库名称。
  - **`drop()`方法**：用于删除用户当前所使用的数据集。
  - **`insert()`方法**：用于插入一条或多条数据，它接收一个JSON数据格式的对象或数组为实参，用于指定要插入的数据。
  ...
  - **`deleteOne()`方法**：用于删除某一条指定的数据，它接收一个JSON数据格式的对象为实参，用于指定要删除的数据。
  - **`deleteMany()`方法**：用于删除多条指定的数据，它接收一个JSON数据格式的数组为实参，用于指定要删除的数据。
  - **`find()`方法**：用于用于查找指定的数据，它接收一个JSON数据格式的对象为实参，用于指定要查找的数据。
  - **`findOne()`方法**：该方法是`find()`方法的特化版本，它只返回查找到的第一条数据，它接收一个JSON数据格式的对象为实参，用于指定要查找的数据。
  - **`update()`方法**：用于更新指定的数据，它接收一个JSON数据格式的对象或数组为实参，用于指定要更新的数据。
  - **`updateMany()`方法**：该方法是`update()`方法的特化版本，专用于更新多条数据，它接收一个JSON数据格式的数组为实参，用于指定要更新的数据。
  - **`count()`方法**：用于返回当前数据集中符合指定条件的数据有几条，它接收一个JSON数据格式的对象为实参，用于指定要查找的数据。

- **`close()`方法**：用于关闭当前数据库连接。在完成了要执行的数据库操作之后，请读者务必记得使用该方法来关闭数据库连接，否则该数据库连接对象会一直占用计算机的资源。
...
由于本章项目的任务只负责用户功能模块的后端实现，暂时尚未创建与之相应的前端Web UI，所以如果读者想确认自己在这一阶段的工作成果，最简单直接的方法就是使用一款名为CURL的命令行工具来模拟前端可能发送的各种HTTP请求。这里提到的CURL是一个开源的命令行工具，它支持通过HTTP协议来发送请求，并支持通过HTTP协议来接收响应。下面，就让我们根据本章项目的任务去求来简单介绍一下这款命令行工具的使用方法。

- 首先，关于这款命令行工具的获取，由于读者在本章项目中主要使用的是基于Ubuntu+Node.js的开发环境，所以笔者会建议使用`sudo apt install curl`命令来安装该工具。当然了，如果使用的是其他操作系统，读者也可以参考CURL官方网站上的指导来安装这款工具。  

- 在成功安装了CURL之后，读者就可以在命令行终端中利用这款命令行工具来模拟前端发送的各种HTTP请求了。在使用该工具时，我们通常需要用到以下参数。
  - `-X`：该参数用于指定要发送的请求类型，例如`GET`、`POST`、`PUT`、`DELETE`等。
  - `-H`：该参数用于指定要发送的请求头信息，例如`Content-Type: application/json`。
  - `-d`：该参数用于指定要发送的请求体信息，例如`{"name": "张三", "age": 20}`。
  - `-v`：该参数用于指定要显示的请求和响应信息，通常情况下，该参数主要用于针对某一请求的具体调试，
  
举个例子，如果读者现在想向`exmple.com`这个域名发送一系列HTTP请求，用于测试它的用户功能模块，那么他们通常可以通过在命令行终端中执行以下命令来模拟这些HTTP请求。

  ```bash
  # 测试用户注册功能
  curl -X POST -H "Content-Type: application/json" -d '{"name": "张三", "password": "123456", "age":"20"}' http://example.com/users/register
  # 测试用户登录功能
  curl -X POST -H "Content-Type: application/json" -d '{"name": "张三", "password": "123456"}' http://example.com/users/login
  # 测试用户信息查询功能
  curl -X GET http://example.com/users/1
  # 测试用户信息修改功能
  curl -X PUT -H "Content-Type: application/json" -d '{"name": "张三", "age":"21"}' http://example.com/users/1
  # 测试用户信息删除功能
  curl -X DELETE http://example.com/users/1
  ```

需要特别强调的是，本章在这里只是介绍了关于CURL这款命令行工具的基本用法，以便读者能将其用于本章项目的工作成果检验。如果读者想更为全面地了解这款命令行工具的使用方法，那么可以通过在命令行终端中执行`curl -h`命令或者直接去CURL官方网站上查看用户手册来获取更多的详细信息。
...
在这一步骤中，软件工程师的主要任务是将添加了用户功能模块的web服务重新部署到项目【任务书】书中指定的云服务器上，并确认它可以正常运行。当然了，由于本章项目只为该Web服务实现了用户功能模块的后端部分，其相应的Web UI尚未被创建，所以读者目前只能本章在【工作准备】中介绍的命令行工具来模拟网页浏览器向该Web服务发送的各种HTTP请求，以此来确认其返回的响应结果符合预期。为此，读者需要执行以下操作。
...
然而，如果我们选择将这部分工作交给Express.js框架这样的第三方扩展来完成，通常就只需要加载该框架提供的一个路由模块即可。下面，就让我们来演示一下基于Express.js框架来实现HTTP API的基本步骤。

1. 使用VS Code编辑器回到我们在上一章创建的`02_studynodejs/helloExpress`项目中，并在该项目的根目录下创建一个名为`router`的目录，以便用于自定义应用的路由模块。

2. 在刚刚创建的`router`目录下新建一个名为`index.js`的文件，并在其中输入如下代码。
...
3. 回到项目的入口文件中（即`02_studynodejs/helloExpress/index.js`文件）中，将该文件中的代码修改如下。
...
### 知识点2：针对HTTP API的自动化测试

到目前为止，读者在确认工作成果时使用的都是CURL这款命令行工具，但这款工具最多也只能做到“确认”而已。也就是说，在使用CURL模拟前端请求时，由于是人工执行的有限操作，我们通常会选择优先了解后端服务在正常环境中被正确调用时的状况，这样做只能”确认“它实现了相应的功能，但对于它是否能在高负载的情况下面对各种不可预测的前端请求，读者就需要用更为专业的自动化测试工具来模拟用户在前端各种可能的操作了。在Node.js平台中，读者可以通过加载第三方扩展的方式获取到一系列的自动化测试工具，这些工具的使用方式大同小异，下面就以`superTest`库为例来介绍一下这类工具的基本使用步骤。

1. 使用命令行终端环境回到刚刚在上一节中修改过的`02_studynodejs/helloExpress`项目项目的根目录下，并通过执行`npm install supertest --save`命令来将`superTest`库安装到该项目中。

2. 使用VS Code编辑器重新打开`02_studynodejs/helloExpress/index.js`文件，并在该文件的末尾添加一行`module.exports = app`语句，以便之后在创建测试用例时能以模块的方式加载这个被测试的应用。

3. 继续在项目根目录下创建一个名为`test.js`的文件，并在其中输入如下代码。
...
4. 在保存了上述代码之后，读者只需要回到命令行终端环境中，并执行`node ./test.js`命令就可以启动针对这个示例项目自动化测试了。如果一切顺利，读者应该可以看到如图2-4所示的测试结果。
