...
到目前为止，读者在确认工作成果时使用的都是CURL这款命令行工具，但这款工具最多也只能做到“确认”而已。也就是说，在使用CURL模拟前端请求时，由于是人工执行的有限操作，我们通常会选择优先了解后端服务在正常环境中被正确调用时的状况，这样做只能”确认“它实现了相应的功能，但对于它是否能在高负载的情况下面对各种不可预测的前端请求，读者就需要用更为专业的自动化测试工具来模拟用户在前端各种可能的操作了。在Node.js平台中，读者可以通过加载第三方扩展的方式获取到一系列的自动化测试工具，这些工具的使用方式大同小异，下面就以`superTest`库为例来介绍一下这类工具的使用方法。

1. 首先，读者需要使用命令行终端环境回到刚刚在上一节中修改过的`02_studynodejs/helloExpress`项目项目的根目录下，并通过执行`npm install supertest --save`命令来将`superTest`库安装到该项目中。

2. 接下来，读者需要使用VS Code编辑器重新打开`02_studynodejs/helloExpress/index.js`文件，并在该文件的末尾添加一行`module.exports = app`语句，以便之后在创建测试用例时能以模块的方式加载这个被测试的应用。

3. 再接着，读者需要继续在项目根目录下创建一个名为`test.js`的文件，并在其中输入如下代码。
