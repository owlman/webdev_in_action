...
1. 回到之前的`01_snowbear`项目中，然后在项目根目录下的`backend`目录中为创建一个名为`users.js`的文件，并在其中输入如下代码。

    ```JavaScript
    ```

2. 重新打开`01_snowbear/backend`目录下的`index.js`文件，先在文件的开头处添加一行`require`语句，以便将刚刚在`users.js`文件中定义的功能模块引入到当前文件中，具体如下。
...
3. 然后继续在`index.js`文件中找到我们之前为本章项目预留的`apiServer()`函数，并将其实现修改如下。
