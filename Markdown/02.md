...
在这一步骤中，软件工程师的主要任务是创建一个专用于执行MongoDB数据库操作的Node.js模块，该模块所提供的API将会在接下来为凌雪冰熊网站实现各项Web后端服务的过程中被反复调用，目的是通过提供这部分代码的重用率来减少项目整体的代码量，提高开发效率。为此，读者需要执行以下操作。

1. 使用VS Code编辑器打开之前在上一章中创建的`01_snowbear`项目，然后在项目根目录下的`backend`目录中创建一个名为

创建一个名为`useMongodb`的模块目录，并在该目录下创建一个名为`index.js`的文件，然后在该文件中使用`require()`方法将`mongodb`扩展包引入到当前项目中，就可以使用MongoDB数据库了。然后，和使用MySQL数据库时一样，我们在使用`useMongodb`模块中操作MongoDB数据库时也需要先创建一个数据库连接对象，这个对象的创建操作一般是通过实例化`mongodb`扩展包中定义的`MongoClient`类来完成的，为此，我们可以刚刚创建的`useMongodb/index.js`文件中输入如下代码。
...
在保存上述代码之后，如果我们在该模块目录下执行`node index.js`命令时看到它命令行终端中输出内容为“数据库连接成功！”的信息，就说明mongodb扩展包和MongoDB数据库都已经准备就绪了。下面，让我们来具体了解一下这段代码中所做的事情。

首先，我们在实例化`MongoClient`类的对象时提供了一个用于指定数据库所在位置的字符串实参。该字符串中必须要指明数据库服务器所使用的网络协议（即这里的`mongodb://`）、数据库所在服务器的IP地址或域名（即这里的`localhost`）以及该数据库服务使用的端口号（即这里的`27017`）。除此之外，我们很多时候还需要在该字符串中指明在连接数据库时所要使用的用户名和密码，例如下面字符串中的`<my_username>`和`<my_password>`。
...
当然了，在所有的数据操作完成之后，还请读者务必要记得调用该连接对象的`close()`方法来关闭连接数据库连接。下面，我们可以初步来体验一下上面这些方法的使用方式。例如，如果想在`02_onlineResumes`项目中用mongodb扩展包提供的API来构建一个操作MongoDB数据库中的功能模块，就可以回到`useMongodb/index.js`文件中，并接着之前的内容继续编写如下代码：
...
MongodbApi.getResumeByUID = async function(uid) {
    try {
        await client.connect();
        const db = client.db(databaseName);
        const collect = db .collection('resumes');
...
如你所见，虽然上述API的实现中还存在着大量的代码冗余及其可能导致的低效率问题，还需要更进一步的方案优化，但相较于之前操作MySQL数据库的API，它已经简单了不少。至少，我们在使用MongoDB数据管理简历数据的时候并不需要将它的主数据和子数据分开存储，而只需要将它编码成一个JSON格式的对象直接插入数据库中的相应数据集即可，就像下面这样。

```JavaScript
MongodbApi.insert('resumes',{
    "cv_id":202202042,
    "name":"张三",
    "gender":"男",
    "age":"25",
...
这样一来，我们就既免除了复杂的数据库设计工作，也可以在程序运行过程实现一次数据操作解决所有的数据存取问题。不然的话，我们在存储一个数据实体时就必须先存储该实体的子数据，然后在获取到子数据在数据库表中的键值字段，再去存储该实体的主数据。而在查询数据实体时顺序又恰好相反，必须先查询主数据，待获取到相关键值字段之后才能查询子数据。当然了，必须要再次强调的是，在享受非关系型数据库这种弱约束所带来的自由的同时，开发者必须自己承担起维护数据存储结构在一致性方面的责任，毕竟凡事皆有代价，天下没有既免费又美味的午餐。
