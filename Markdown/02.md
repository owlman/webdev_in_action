...
3. 接下来，读者需要关闭上述与云服务器的SSH连接，并继续在命令行环境中依次使用下列命令来模拟网页浏览器向该Web服务发送HTTP请求，以便执行用户注册、用户登录、用户查询和用户修改等操作。
    ...
    ```bash
    # 模拟执行用户注册操作的 POST 请求
    curl -X POST -d '{"username":"snowbear","password":"123456"}' http:/localhost/users/newuser
    # 模拟执行用户登录操作的 POST 请求
    curl -X POST -d '{"username":"snowbear","password":"123456"}' http://localhost/users/session
    # 模拟执行按 ID 查询用户操作的 GET 请求，确认用户注册结果
    curl -X GET http://localhost/users/1
    # 模拟执行用户修改操作的 PUT 请求
    curl -X PUT -d '{"username":"snowbear","password":"123456","address":"xx市xx街xx号","phone":"1011010","email":"my@snowbear.com"}' http://localhost/users/1
...
正如读者所见，本章项目将近一半的工作量都放在了针对前端请求的处理上。具体来说，读者首先要创建一个自定义模块（即`httpApi`模块），然后在其中定义`getRequest()`、`postRequest()`、`putRequest()`和`deleteRequest()`四个用于处理HTTP请求的API，分别用于处理前端使用GET、POST、PUT、DELETE四种方法发来的各种请求。最后回到项目后端部分的入口文件中（即`01_snowbear/backend/app.js`文件），通过`apiServer()`函数将这些请求分发给该模块中相应的API来处理。但是，如果我们选择将这部分工作交给Express.js框架来完成，就只需要加载该框架的一个路由模块即可。下面，我们就来演示一下基于Express.js框架来实现HTTP API的方法。
