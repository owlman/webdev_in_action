...
- **项目资料**：用户信息管理功能的后端API，其具体信息如下。
  ...
  - 响应数据，以JSON格式返回：
    ...
    - 失败响应1: `{ status: 404, message: "user_not_found"}`
...
1. 使用VS Code编辑器重新打开`scripts`目录下的`commonForm.js`文件，并在其中添加如下函数，以便用于检查电子邮件、电话号码、收货地址的输入格式。
...
2. 另外在`scripts`目录创建一个名为`userinfo.js`的脚本文件，并根据在`userInfo.htm`文件中所设置的表单元素在其中输入如下代码，以便完成对当前Web UI的输入检查。

    ```javascript
    ```

3. 接下来，读者需要在`userinfo.js`文件中为用于上传头像的`<input type="file">`元素注册`change`事件的处理函数，以便将用户上传的图形文件序列化为`Base64`编码的字符串，并将该图形回显在界面中。该处理函数的具体实现代码如下。

    ```javascript
    ```

4. 接着在`userinfo.js`文件中分别为`id`属性值为`baseinfoForm`、`passwordForm`、`deleteForm`的表单元素注册`submit`事件的处理函数，以便在用户提交表单时将他们输入的信息统合成单一的数据对象并将其序列化为JSON格式的字符串，然后再将该字符串以HTTP请求的形式发送给后端API，并根据后端的响应做出相应的处理。该处理函数的具体实现代码如下。

    ```javascript
    ```

5. 继续在`userinfo.js`脚本文件中为用于删除用户信息的按钮元素注册`click`事件的处理函数，以便让当前Web应用在用户点击该按钮时向后端发送HTTP请求，并调用相应的API来删除当前用户的个人信息。该处理函数的具体实现代码如下。

    ```javascript
    ```

6. 在保存了上述所有文件之后，读者就重新启动这个Web应用，并经由登录界面进入到这个用户个人信息的编辑界面，并对其功能进行一系列测试了（关于这部分测试的自动化，我们稍后会将其作为本项目的【拓展知识】介绍给读者），其效果如图4-4所示。

    ![用户信息管理功能的前端实现](./img/4-4.png)

    **图4-4** 用户信息管理功能的前端实现

7. 如果上述操作一切顺利，读者就可以将当前步骤所获得的进展提交给git版本控制系统了，具体命令如下。
