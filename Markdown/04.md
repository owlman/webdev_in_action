...
1. 使用VS Code编辑器重新打开我们在上一章中创建的`userinfo.htm`文档，并基于凌雪冰熊项目既定的网页模板（即`frontend/static/template.htm`文件）将其初始化为如下HTML文档。
...
2. 在`userinfo.htm`文档找到`<!-- 在此处填充网页的主体内容 -->`这行注释所在的位置，并在该处添加如下代码，以便创建用于查看/编辑用户信息的Web UI。

    ```html
    ```

3. 使用VS Code编辑器重新打开我们在上一章中创建的`login&signup.js`脚本文件，在其中找到`checkPassword`函数，并在该函数的定义之后添加如下`export`语句，以便导出其中可在本项目中重用的表单处理函数。
...
4. 根据上述HTML文档中`<script>`标签中的设置，在指定的`scripts`目录下创建一个名为`userInfo.js`的前端脚本文件，并在其中输入如下代码。
