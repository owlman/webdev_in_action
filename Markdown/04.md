...
用户信息管理功能的前端实现在Web应用开发领域中属于最简单的信息管理类UI项目，此类项目的开发目的是让企业的用户可以通过网页浏览器来编辑使用其线上服务时需要提供的个人信息，例如个人头像、电话号码、收货地址等。除了基于B/S架构的用户界面所能带来的低成本效益之外，此类功能的实现还能让用户能以较为便捷的方式提供这些信息，是企业得以开展电子商务等互联网业务的基础，因此它的开发也通常被认为是软件工程师在进入到Web应用开发领域时在前端部分必须掌握的项目类型之一。
...
在上一章的项目实践中，读者通过学习如何实现用户注册/登录功能的项目，初步掌握了在基于JavaScript语言的前端脚本中处理字符串类型数据的方法。然而在本章项目将要实现的用户信息管理功能，乃至于一些更为复杂的应用场景中，前端脚本要处理的可远不止字符串这一种数据类型，用户要在Web UI中输入并提交的信息中往往还会包含数字、时间、布尔值、图形等各种不同类型的数据。这些数据通常都必须先被序列化成某种特定格式的数据之后，才能被提交给Web应用的后端服务。因此，要想在前端脚本中处理不同类型的数据，开发者们就必须更进一步地学习如何利用网页浏览器提供的接口来存储并处理这些数据。在本章项目的【工作准备】部分中，笔者将会对相关的知识点进行着重介绍。和之前一样，如果读者觉得自己已经掌握了上述知识，也可以选择跳过本节内容，直接进入本章项目的【工作实施与交付】环节。
...
8. 在保存了上述所有文件之后，如果读者重新启动这个Web应用，在成功完成登录操作之后，就可以在网页浏览器中看到用于编辑用户信息的初始界面了，其效果如图4-3所示。
...
- 至此，读者已经利用Vue.js框架完成了表单元素与前端数据对象的双向绑定，接下来只需要重新启动该项目的开发服务器，读者就可以在网页浏览器中观察到，用户在Web UI中输入到的信息会同步更新到`userData`这个前端数据对象中了，效果如图4-8所示。

    ![基于Vue.js的Web UI](./img/4-8.png)

    **图4-8** 基于Vue.js实现的Web UI

从上述实例的演示中，读者可以初步了解Vue.js前端项目的组织方式。总体而言，Vue.js框架采用的是一种树状的组件化构建方式，其中的每个组件都由一个扩展名为`.vue`的文件来描述，其内容通常会包含`<template>`、`<script>`和`<style>`这三个Vue标记，开发者们将会分别用它们来编写以下三种不同用处的代码。

- **`<template>`标记**：用于编写带有`v-if`、`v-for`和`v-Model`等Vue专有属性指令的HTML代码，这些代码可以被看作是组件的Web UI模版，功能是描述我们想要在该组件中呈现的界面元素，以及这些元素所绑定的数据对象。例如在上述示例中，我们基于`<form>`标记定义了一个用于编辑用户基本信息的界面，并利用`v-model`指令将该界面中的各输入项与`userData`这个前端数据对象中的各属性做了双向绑定。
- **`<script>`标记**：用于编写JavaScript代码，这些代码可以被看作组件的前端脚本，功能是负责定义该组件要使用的前端数据对象，以及用于响应用户操作的事件处理函数。例如在上述示例中，我们利用Vue.js框架提供的`reactive`函数定义了`userData`这个前端数据对象，并将其设置为可供界面元素双向绑定的响应式对象。
- **`<style>`标记**：用于编写CSS样式代码，这些代码可以被看作是组件的专属样式，功能是设置该组件要呈现的外观样式。在上述示例中，我们并没有为`baseInfo.vue`组件编写任何专属的样式代码，所以该组件的默认样式就是Bootstrap框架所提供的样式。

当然，上述实例中所演示的只是将组件的界面元素与其前端数据对象进行双向绑定的方法。如果想让这个`baseInfo.vue`组件成为能真正用于编辑用户基本信息的Web UI，读者还需要为该组件编写相应的事件处理函数，关于这方面的内容，本教材将留待后续章节中再做详细介绍。在这里，读者只需要初步了解如何利用Vue.js框架来构建前端项目，以及它在实现表单元素与数据对象的双向绑定 方面所能带来的便利即可。

#### 第3步：生成静态页面

在上述示例中，我们一直是借由`npm run dev`命令启动的开发服务器来验证项目开发成果的。该开发服务器是一个用于动态编译项目，并热部署其生成结果的开发/调试环境，它在通常情况下并不会将项目的编译结果保存下来。如果读者需要将项目生成的前端代码真正部署到一个Web服务器上，就需要另外在项目的根目录下执行`npm run build`命令，后者会当前目录下生成一个名为`dist`的目录，该目录中存放的就是部署Web服务所需要的HTML+CSS+JavaScript的前端实现源码，具体如图4-9所示。

![Vue.js项目的输出](./img/4-9.png)

**图4-9** 获取Vue.js项目的输出结果

## 【作业】

有一家名为“白熊前端”的程序员培训机构，他们决定为自己现有的Web服务添加一个用户功能模块，以便用户可以通过互联网申请成为他们的学员，并能在登录之后享用他们的线上培训服务，在开发该功能模块的项目中，你的任务是根据项目组中负责后端部分的成员所实现的HTTP API来构建用于执行用户信息管理操作的Web UI。

- **项目名**：白熊培训网站用户信息管理的Web UI
- **委托方**：白熊前端的创始人：林宇一
- **项目资料**：用户信息管理功能的后端API，其具体信息如下。
  - *用户信息修改的API*：
    - 请求URL：`http://whitebear.com/users/<用户的ID>`
    - 请求方法：`PUT`
    - 请求参数，需以JSON格式提交, 具体数据为用户所希望修改的个人信息。
    - 响应数据，以JSON格式返回：
      - 成功响应：`{ status: 200, message: "user_update_success"}`
...
| 03 | 编辑/删除用户信息的功能 | Web UI能否在输入正确的情况下完成它的功能？| 40   |  |  |  |  |
