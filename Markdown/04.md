...
在拥有用户信息管理模块的Web应用中，允许用户设置头像是一个开发者们常需要实现的功能。由于用来充当头像的通常是一些尺寸相对较小的图像文件，人们在实现该功能时往往会选择以二进制字节流的形式提交给后端服务，因此，如何在前端脚本中将用户上传的图像文件转换成二进制的字节流，是读者在实施本章项目之前必须要掌握的一项基本技能。

如果读者使用的是JavaScript语言，这项技能可以通过学习如何实现图像数据与`Base64`字符串之间的转换来获得。下面，让我们具体来介绍一下如何将用户上传的图像文件序列化成`Base64`格式的字符串，先介绍基于`FileReade`对象的实现方法，基本步骤如下。
...
- 再根据上述HTML文档中`<script>`标签的设置，在指定的`/scrip/`目录下创建一个名为`imageToBase64.js`的前端脚本文件，并在其中输入如下代码。
