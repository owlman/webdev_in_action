...
在这一步骤中，软件工程师的主要任务是基于HTML构建出用于查看、修改和删除用户信息的Web UI界面，并通过编写前端脚本的方式对用户在该Web UI中的输入进行即时检查，以便排除一些不符合规则的输入，提升Web应用的安全性。为此，读者需要执行以下操作。
...
4. 在代码编辑器中打开上述`userinfo.htm`文档，在其中找到`<!-- 在此处填充网页的主体内容 -->`这行注释所在的位置，并在该处添加如下代码，以便创建用于编辑用户个人信息的Web UI。

    ```html
    ```

5. 根据上述HTML文档中所设置的表单元素，重新打开`scripts`目录下的`commonForm.js`文件，并在其中添加如下代码。

    ```javascript
    ```

6. 继续在`scripts`目录另外创建一个名为`userinfo.js`的脚本文件，并在其中输入如下代码，以便完成对当前Web UI的输入检查。
