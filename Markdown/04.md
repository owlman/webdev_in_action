...
在上述前端脚本中，我们具体执行了以下操作。、

- 首先，我们通过`document.querySelector()`方法获取了表单元素`<form>`，并将其保存在变量`inputForm`中。
- 然后，我们为`inputForm.photo`输入项注册了`change`事件的处理函数，并在该处理函数中将用户上传的头像转换成了`Base64`编码，并将其保存在`formData`对象中。
- 接下来，我们又为`inputForm`注册了`submit`事件的处理函数，并在该处理函数中获取了用户输入的用户名和个人简介，并将它们保存在`formData`对象中。
