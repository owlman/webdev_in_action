...
1. 使用VS Code编辑器重新打开之前刚刚修改过的`login&signup.js`脚本文件，将其中`addHint`、`removeHint`、`removeValidation`、`checkUsername`和`checkPassword`这四个函数的定义提取出来另存为一个名为`commonForm.js`的j脚本文件，以便在本项目中继续重用这些函数。

2. 使用VS Code编辑器重新打开我们在上一章中创建的`login&signup.htm`文档，在其中找到找到`<!-- 引入自定义的JavaScript脚本文件 -->`这行注释所在的位置，并在该位置与加载`login&signup.js`脚本文件的`<script>`标记之间插入用于加载`commonForm.js`脚本的`<script>`标记，具体如下。
...
3. 使用VS Code编辑器重新打开我们在上一章中创建的`userinfo.htm`文档，并基于凌雪冰熊项目既定的网页模板（即`frontend/static/template.htm`文件）将其初始化为如下HTML文档。
...
4. 在`userinfo.htm`文档找到`<!-- 在此处填充网页的主体内容 -->`这行注释所在的位置，并在该处添加如下代码，以便创建用于查看/编辑用户信息的Web UI。

    ```html
    ```

5. 根据上述HTML文档中所设置的表单元素，重新打开`scripts`目录下的`commonForm.js`文件，并将其代码修改如下。

    ```javascript
    ```

6. 继续在`scripts`目录创建一个名为`userinfo.js`的脚本文件，并在其中输入如下代码，以便完成对当前Web UI的输入检查。

    ```javascript
    ```

7. 在保存了上述所有文件之后，如果读者重新启动这个Web应用的登录界面，在成果完成登录操作之后，就可以在网页浏览器中看到用于查看/编辑用户个人信息的Web UI了，其效果如图4-3所示。

    ![用户个人信息的编辑界面](./img/4-3.png)

    **图4-3** 用户个人信息的编辑界面

8. 如果上述操作一切顺利，读者就可以将当前步骤所获得的进展提交给git版本控制系统了，具体命令如下。
...
1. 使用VS Code编辑器重新打开`scripts`目录下的`userinfo.js`文件，并在其中为用于设置用户头像的表单项注册`change`事件的处理函数，用于将用户上传的图形文件保存为`Base64`编码格式的字符串。该处理函数的具体实现代码如下。

    ```javascript
    ```

2. 继续在上述`userinfo.js`脚本文件中为用于充当用户个人信息编辑界面的表单元素注册`submit`事件的处理函数，以便在用户点击提交按钮时，将用户在表单中输入的信息统合成单一的数据对象，并在对其进行序列化处理之后以HTTP请求的形式发送给后端的API，然后再根据后端的响应做出相应的处理。该处理函数的具体实现代码如下。

    ```javascript
    ```

3. 继续在`userinfo.js`脚本文件中为用于删除用户信息的按钮元素注册`click`事件的处理函数，以便让当前Web应用在用户点击该按钮时向后端发送HTTP请求，并调用相应的API来删除当前用户的个人信息。该处理函数的具体实现代码如下。

    ```javascript
    ```

4. 在保存了上述所有文件之后，读者就重新启动这个Web应用，并经由登录界面进入到这个用户个人信息的编辑界面，并对其功能进行一系列测试了（关于这部分测试的自动化，我们稍后会将其作为本项目的【拓展知识】介绍给读者），其效果如图4-4所示。

    ![用户信息管理功能的前端测试](./img/4-4.png)
